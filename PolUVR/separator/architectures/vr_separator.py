...
from PolUVR.separator.uvr_lib_v5.vr_network import nets, nets_new
...
class VRSeparator(CommonSeparator):
    ...
    def __init__(self, common_config, arch_config: dict):
        ...
        if self.post_process_threshold < 0.1:
            self.logger.warning(
                f"post_process_threshold {self.post_process_threshold} is less than the minimum allowed value of 0.1. Setting post_process_threshold to 0.1."
            )
            self.post_process_threshold = 0.1
        elif self.post_process_threshold > 0.3:
            self.logger.warning(
                f"post_process_threshold {self.post_process_threshold} is greater than the maximum allowed value of 0.3. Setting post_process_threshold to 0.3."
            )
        ...
        if self.batch_size < 1:
            self.logger.warning(
                f"batch_size {self.batch_size} is less than the minimum allowed value of 1. Setting batch_size to 1."
            )
            self.batch_size = 1
        elif self.batch_size > 16:
            self.logger.warning(
                f"batch_size {self.batch_size} is greater than the maximum allowed value of 16. Setting batch_size to 16."
            )
            self.batch_size = 16
        ...
        if self.window_size < 320:
            self.logger.warning(
                f"window_size {self.window_size} is less than the minimum allowed value of 320. Setting window_size to 320."
            )
            self.window_size = 320
        elif self.window_size > 1024:
            self.logger.warning(
                f"window_size {self.window_size} is greater than the maximum allowed value of 1024. Setting window_size to 1024."
            )
        ...
        if aggression_value < 0:
            self.logger.warning(
                f"aggression {aggression_value} is less than the minimum allowed value of 0. Setting aggression to 0."
            )
            aggression_value = 0
        elif aggression_value > 100:
            self.logger.warning(
                f"aggression {aggression_value} is greater than the maximum allowed value of 100. Setting aggression to 100."
            )
